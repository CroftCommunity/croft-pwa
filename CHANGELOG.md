...
- 2026-09-08 **pds-walker:** croft-pwa is now also a **package**. `npm run build:lib` emits
  ...
  install through `prepare`; consumers pin `github:CroftCommunity/croft-pwa#<sha>`
  (`CroftC/.claude/SHARED-CODE.md` rule 2). The library's own version clock is `VERSION`
  (0.1.0), released by `pds-walker-vX.Y.Z` tags through `release-pds-walker.yml`; this file
  now declares two contexts. The walker itself follows (`plans/2026-09-08-plan-pds-walker.md`).

## 2026-08

- 2026-08-30 **site:** Sign in at **Bluesky** from the sheet works. The Bluesky row failed with
...
- 2026-08-29 **site:** The atproto page signs in through the workspace **sign-in sheet**: "Choose
...
- 2026-08-29 **site:** The dependency gate decides **inbound licences** as well as CVEs, from one
...
- 2026-08-29 **site:** The gate no longer reports "could not run" against a lockfile that
...
- 2026-08-29 **site:** The dependency gate reads `requirements.txt` too. It did not, and
...
- 2026-08-29 **site:** `docs/CI.md` rule 9 — the shared security workflow, and the dependency
...
- 2026-08-29 **site:** `.github/workflows/security-reusable.yml` — the workspace secret-scanning
...
- 2026-08-29 **site:** Relicensed MIT → AGPL-3.0, matching the rest of the workspace (owner decision).
- 2026-08-29 **site:** Accessibility requirement 4: an axe exclusion must match something — a scope
  that matches nothing is a scan that silently grades a different page.
- 2026-08-29 **site:** The `ing.croft.croftpwa.*` lexicon register (workspace LEXICONS dimension).
- 2026-08-28 **site:** `docs/PWA-INSTALL.md` — what an installed PWA does between the tap and the
  first frame; the splash you control is a page, not a manifest field.
- 2026-08-26 **site:** The tap-target floor is 44px (WCAG 2.5.5), and a `scrollWidth` overflow check
